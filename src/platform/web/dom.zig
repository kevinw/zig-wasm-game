...
fn launch() !void {
    ...
    if (body_node == 0) return error.QuerySelectorError;

    // We don't release this one as we'll be referencing it later
    const input_tag_name = "input";
    input_tag_node = create_element(&input_tag_name, input_tag_name.len);
    if (input_tag_node == 0) return error.CreateElementError;

    const input_tag_attr_name = "value";
    const input_tag_attr_value = "Hello from Zig!";
    set_attribute(input_tag_node, &input_tag_attr_name, input_tag_attr_name.len, &input_tag_attr_value, input_tag_attr_value.len);

    const button_tag_name = "button";

    const submit_button_node = create_element(&button_tag_name, button_tag_name.len);
    defer release_object(submit_button_node);
    if (submit_button_node == 0) return error.CreateElementError;
    ...
    if (submit_text_node == 0) return error.CreateTextNodeError;
    ...
    if (attached_submit_text_node == 0) return error.AppendChildError;


    const clear_button_node = create_element(&button_tag_name, button_tag_name.len);
    defer release_object(clear_button_node);
    if (clear_button_node == 0) return error.CreateElementError;
    ...
    if (clear_text_node == 0) return error.CreateTextNodeError;
    ...
    if (attached_clear_text_node == 0) return error.AppendChildError;


    const attached_input_node = append_child(body_node, input_tag_node);
    defer release_object(attached_input_node);
    if (attached_input_node == 0) return error.AppendChildError;
    ...
    if (attached_submit_button_node == 0) return error.AppendChildError;


    const attached_clear_button_node = append_child(body_node, clear_button_node);
    defer release_object(attached_clear_button_node);
    if (attached_clear_button_node == 0) return error.AppendChildError;
}
...
fn on_submit_event() void {
    ...
    const input_tag_attr_name = "value";
    const success = get_attribute(input_tag_node, &input_tag_attr_name, input_tag_attr_name.len, &attribute_ptr, &attribute_len);
